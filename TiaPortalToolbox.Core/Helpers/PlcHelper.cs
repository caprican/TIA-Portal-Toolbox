...
public static class PlcHelper
{
    ...
    private static List<Models.ProjectTree.Plc.Object>? GetPlcBlockGroups(PlcBlockUserGroupComposition plcBlockGroups, string? parentPath)
    {
        ...
        if (plcBlockGroups?.Count > 0)
        {
            ...
            do
            {
                ...
                if (GetPlcBlocks(blockGroup[0].Blocks, System.IO.Path.Combine(groupPath, blockGroup[0].Name)) is List<Models.ProjectTree.Plc.Blocks.Object> blocks)
                {
                    groupItems ??= [];
                ...
                if (blockGroup[0].Groups?.Count > 0 &&
                    GetPlcBlockGroups(blockGroup[0].Groups, System.IO.Path.Combine(groupPath, blockGroup[0].Name)) is List<Models.ProjectTree.Plc.Object> lstGroup)
                {
                    groupItems ??= [];
                ...
                {
                    groups ??= [];
                    groups.Add(new Models.ProjectTree.Plc.Item(blockGroup[0].Name, System.IO.Path.Combine(groupPath, blockGroup[0].Name))
                    {
                        Items = groupItems
                    });
                }
    ...
    private static List<Models.ProjectTree.Plc.Object>? GetPlcTypesGroups(PlcTypeUserGroupComposition plcTypesGroups, string parentPath)
    {
        ...
        if (plcTypesGroups?.Count > 0)
        {
            ...
            {
                ObservableCollection<Models.ProjectTree.Object>? groupItems = null;
                if (GetPlcTypes(typeGroup[0].Types, System.IO.Path.Combine(groupPath, typeGroup[0].Name)) is List<Models.ProjectTree.Plc.Type> blocks)
                {
                    groupItems ??= [];
                    groupItems.AddRange(blocks);
                }
                ...
                if (typeGroup[0].Groups?.Count > 0 &&
                    GetPlcTypesGroups(typeGroup[0].Groups, System.IO.Path.Combine(groupPath, typeGroup[0].Name)) is List<Models.ProjectTree.Plc.Object> lstGroup)
                {
                    groupItems ??= [];
                    groupItems.AddRange(lstGroup);
                }
                ...
                {
                    groups ??= [];
                    groups.Add(new Models.ProjectTree.Plc.Item(typeGroup[0].Name, System.IO.Path.Combine(groupPath, typeGroup[0].Name))
                    {
                        Items = groupItems
                    });
                }
    ...
    private static List<Models.ProjectTree.Plc.Object>? GetPlcTagsGroups(PlcTagTableUserGroupComposition plcTagsGroups, string parentPath)
    {
        ...
        if (plcTagsGroups?.Count > 0)
        {
            ...
            {
                ObservableCollection<Models.ProjectTree.Object>? groupItems = null;
                if (GetPlcTags(tagGroup[0].TagTables, System.IO.Path.Combine(groupPath, tagGroup[0].Name)) is List<Models.ProjectTree.Plc.Tag> blocks)
                {
                    groupItems ??= [];
                    groupItems.AddRange(blocks);
                }
                ...
                if (tagGroup[0].Groups?.Count > 0 &&
                    GetPlcTagsGroups(tagGroup[0].Groups, System.IO.Path.Combine(groupPath, tagGroup[0].Name)) is List<Models.ProjectTree.Plc.Object> lstGroup)
                {
                    groupItems ??= [];
                    groupItems.AddRange(lstGroup);
                }
                ...
                {
                    groups ??= [];
                    groups.Add(new Models.ProjectTree.Plc.Item(tagGroup[0].Name, System.IO.Path.Combine(groupPath, tagGroup[0].Name))
                    {
                        Items = groupItems
                    });
                }
    ...
    private static List<Models.ProjectTree.Plc.Object>? FindPlcBlockGroups(PlcBlockUserGroupComposition plcBlockGroups, string? parentPath, string blockName)
    {
        ...
        if (plcBlockGroups?.Count > 0)
        {
            ...
            {
                ObservableCollection<Models.ProjectTree.Object>? groupItems = null;
                if (FindPlcBlocks(blockGroup[0].Blocks, System.IO.Path.Combine(groupPath, blockGroup[0].Name), blockName) is List<Models.ProjectTree.Plc.Blocks.Object> blocks)
                {
                    groupItems ??= [];
                    groupItems.AddRange(blocks);
                }
                ...
                if (blockGroup[0].Groups?.Count > 0 &&
                    FindPlcBlockGroups(blockGroup[0].Groups, System.IO.Path.Combine(groupPath, blockGroup[0].Name), blockName) is List<Models.ProjectTree.Plc.Object> lstGroup)
                {
                    groupItems ??= [];
                    groupItems.AddRange(lstGroup);
                }
    ...
    private static List<Models.ProjectTree.Plc.Object>? FindPlcTypesGroups(PlcTypeUserGroupComposition plcBlockGroups, string? parentPath, string blockName)
    {
        ...
        if (plcBlockGroups?.Count > 0)
        {
            ...
            {
                ObservableCollection<Models.ProjectTree.Object>? groupItems = null;
                if (FindPlcTypes(blockGroup[0].Types, System.IO.Path.Combine(groupPath, blockGroup[0].Name), blockName) is List<Models.ProjectTree.Plc.Type> blocks)
                {
                    groupItems ??= [];
                    groupItems.AddRange(blocks);
                }
                ...
                if (blockGroup[0].Groups?.Count > 0 &&
                    FindPlcTypesGroups(blockGroup[0].Groups, System.IO.Path.Combine(groupPath, blockGroup[0].Name), blockName) is List<Models.ProjectTree.Plc.Object> lstGroup)
                {
                    groupItems ??= [];
                    groupItems.AddRange(lstGroup);
                }
    ...
    private static List<Models.ProjectTree.Plc.Object>? FindPlcTagsGroups(PlcTagTableUserGroupComposition plcBlockGroups, string? parentPath, string blockName)
    {
        ...
        if (plcBlockGroups?.Count > 0)
        {
            ...
            {
                ObservableCollection<Models.ProjectTree.Object>? groupItems = null;
                if (FindPlcTags(blockGroup[0].TagTables, System.IO.Path.Combine(groupPath, blockGroup[0].Name), blockName) is List<Models.ProjectTree.Plc.Tag> blocks)
                {
                    groupItems ??= [];
                    groupItems.AddRange(blocks);
                }
                ...
                if (blockGroup[0].Groups?.Count > 0 &&
                    FindPlcTagsGroups(blockGroup[0].Groups, System.IO.Path.Combine(groupPath, blockGroup[0].Name), blockName) is List<Models.ProjectTree.Plc.Object> lstGroup)
                {
                    groupItems ??= [];
                    groupItems.AddRange(lstGroup);
                }
