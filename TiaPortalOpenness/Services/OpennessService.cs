...
using TiaPortalOpenness.Models.ProjectTree.Devices;
...
public class OpennessService : Contracts.Services.IOpennessService
{
    ...
    private void TiaPortal_Disposed(object sender, EventArgs e)
    {
        if (tiaPortal is not null)
        {
            tiaPortal.Confirmation -= TiaPortal_Confirmation;
    ...
    {
        var tiaProcesses = new List<TiaPortalToolbox.Core.Models.TiaProcess>();
        foreach (var process in TiaPortal.GetProcesses())
        {
            tiaProcesses.Add(new TiaPortalToolbox.Core.Models.TiaProcess(process.Id, process.ProjectPath));
        }
        return tiaProcesses;
    }
    ...
    private static void GetDeviceList(ref List<Device> devices, Project project)
    {
    ...
    private static void recurciveGetDeviceList(ref List<Device> devices,
    DeviceUserGroup devicegroup)
    {
        foreach (Device device in devicegroup.Devices)
        {
            devices.Add(device);
        }
        foreach (DeviceUserGroup devicesubgroup in devicegroup.Groups)
        {
